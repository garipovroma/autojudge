...
def run_spec_dec_collect_stats(sample_idx, question_sample, device, target_model, draft_model, tokenizer, K, args, eval_sample):
...
def load_split(args):
    ...
    import os    
    split_ids = torch.load(args.split_path)[int(os.environ['FOLD_IDX'])]
...
def main(args):
    ...
    logger.info(f'Loading split from {args.split_path} (none stands for eval using all data)')
    if args.split_path.lower() != 'none':
